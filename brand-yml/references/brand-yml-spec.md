## brand.yml Specification Reference
...
follow-up instructions in the YAML as inline comments.heme`: Shiny theme defaults and rules.

## Best Practices

- When brands define a range of shades and tints, choose the midpoint color as the primary color in `color.palette`.
- For logo and font files, use placeholders in `_brand.yml` with instructions to download files and place them next to `_brand.yml`.
- Suggest substitutes for proprietary fonts by finding the closest equivalent on Google Fonts.
- Prefer hex color syntax for raw color values.
- Keep the file simple and concise. Only include fields that directly apply to the brand.
- Use the simple string form when possible instead of the mapping form.
- All fields are optional. Do not pad the file with unnecessary defaults.
